...
export const handlers = [
  ...
  // ============================================
  // 마이페이지 관련 새로 추가된 API 핸들러들
  // ============================================

  // 이미지 업로드 API (마이페이지용)
  rest.post('/api/images/upload', async (_req, res, ctx) => {
    console.log('📤 MSW: 프로필 이미지 업로드');
    
    // FormData를 mock으로 처리
    return res(
      ctx.status(200),
      ctx.json('https://picsum.photos/100/100?random=' + Math.floor(Math.random() * 1000))
    );
  }),
